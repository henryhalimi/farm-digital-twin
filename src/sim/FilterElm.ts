...
export class FilterElm extends CircuitElm {
  ...
  getXmlDumpType(): string { return 'fi' }
  ...
  draw(dc: DrawContext, onLoad?: () => void): void {
    ...
    const img = getImage('/filter-icon.svg', onLoad)
    if (isReady(img)) {
  ...
  static fromXml(elem: Element): FilterElm {
    const x = elem.getAttribute('x')!.split(' ').map(Number)
    const elm = new FilterElm(x[0], x[1], x[2], x[3])
    elm.undumpXml(elem)
    elm.setPoints()
    return elm
  }
